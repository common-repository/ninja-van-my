...
class TCPDFBarcode {
	...
	public function getBarcodeSVGcode($w=2, $h=30, $color='black') {
		...
		$svg .= '<svg width="'.esc_attr(round(($this->barcode_array['maxw'] * $w), 3)).'" height="'.esc_attr($h).'" version="1.1" xmlns="http://www.w3.org/2000/svg">'."\n";
		$svg .= "\t".'<desc>'.htmlspecialchars($this->barcode_array['code']).'</desc>'."\n";
		$svg .= "\t".'<g id="bars" fill="'.esc_attr($color).'" stroke="none">'."\n";
		// print bars
		$x = 0;
		foreach ($this->barcode_array['bcode'] as $k => $v) {
			...
			if ($v['t']) {
				...
				// draw a vertical bar
				$svg .= "\t\t".'<rect x="'.esc_attr($x).'" y="'.esc_attr($y).'" width="'.esc_attr($bw).'" height="'.esc_attr($bh).'" />'."\n";
			}
			$x += $bw;
		}
		$svg .= "\t".'</g>'."\n";
	...
	public function getBarcodeHTML($w=2, $h=30, $color='black') {
		$html = '<div style="font-size:0;position:relative;width:'.esc_attr($this->barcode_array['maxw'] * $w).'px;height:'.esc_attr($h).'px;">'."\n";
		// print bars
		$x = 0;
		foreach ($this->barcode_array['bcode'] as $k => $v) {
			$bw = round(($v['w'] * $w), 3);
			...
			if ($v['t']) {
				$y = round(($v['p'] * $h / $this->barcode_array['maxh']), 3);
				// draw a vertical bar
				$html .= '<div style="background-color:'.esc_attr($color).';width:'.esc_attr($bw).'px;height:'.esc_attr($bh).'px;position:absolute;left:'.esc_attr($x).'px;top:'.esc_attr($y).'px;">&nbsp;</div>'."\n";
			}
			$x += $bw;
		}
		$html .= '</div>'."\n";
